    ...
    foreach ($discussions as $discussionid => $discussion) {
        ...
        if ($searchforumtitles && coursesearch_mb_stripos($discussion->name, $query) !== false) {
            ...
            if (isset($firstposts[$discussionid])) {
                ...
                if (coursesearch_is_highlight_enabled() && !empty($query)) {
                    $discussionurl->param('cs_highlight', $query);
                }
...
/**
 * Build renderable search_result objects from raw search results.
 *
 * Shared by the activity view and the course-level search page so that both render results
 * identically. Depends only on the course and the query - it needs no activity instance.
 *
 * @param array $rawresults Raw results from coursesearch_perform_search()
 * @param object $course The course the results belong to
 * @param string $query The search query
 * @return \mod_coursesearch\output\search_result[] Renderable result objects
 */
function coursesearch_build_result_objects(array $rawresults, $course, $query) {
