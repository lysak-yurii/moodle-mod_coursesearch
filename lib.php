...
function coursesearch_get_coursemodule_info($coursemodule) {
    ...
    // This ensures scrolling works when a coursesearch module exists in the course
    if ($PAGE->pagetype === 'course-view') {
        ...
        (function() {
            ...
            if (document.readyState === 'loading') {
                document.addEventListener('DOMContentLoaded', function() { setTimeout(initScroll, 500); });
            } else {
        ...
        ";
        // Output the script directly - js_init_code doesn't work reliably
        // We'll use a different approach - add it to the page footer
        // Store it in a global variable that will be output in footer
        if (!isset($GLOBALS['coursesearch_scroll_script'])) {
            $GLOBALS['coursesearch_scroll_script'] = '';
        }
        $GLOBALS['coursesearch_scroll_script'] = $scrollScript;
    }
